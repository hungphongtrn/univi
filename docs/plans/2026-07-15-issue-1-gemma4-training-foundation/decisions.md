...
## 2026-07-17: Fix image-token count mismatch; raise sequence budget to 8192 and add pre-collator length filter

**Context:** Training at `batch_size > 1` failed immediately with `ValueError: Mismatch in 'image' token count between text and input_ids`. The Gemma4 processor expands each `<|image|>` placeholder into `boi + N×image_token + eoi` (N ≤ 280 soft tokens) before tokenizing. When the collator passed `truncation=True, max_length=4096` to the processor, FineWeb-Edu examples (up to 48K text tokens + 36 images, built with `max_output_tokens=None`) overflowed after expansion; truncation clipped image sub-tokens from `input_ids` but not from the text string, and `_check_special_mm_tokens` caught the mismatch. The project had a custom `collator_max_length` parameter (defaulting to 4096) separate from `SFTConfig.max_length` (2048) — neither value was sufficient, and the parameter does not exist in the original Unsloth guide.

**Decision:** Remove `collator_max_length` entirely; pass `max_length` directly to the Unsloth collator as `max_seq_length` (matching the Unsloth guide pattern). Raise `max_length` from 2048 to 8192 in all configs and in the `config.py` default. Add `_filter_training_tokens()` which drops training and evaluation rows where `original_token_length + num_images×282 + 256 > max_length` before they reach the collator, preventing any truncation of expanded image tokens.

**Rationale:** No finite `max_length` alone eliminates the error — the dataset contains extreme outliers (48K text tokens, 36 images). The pre-collator filter is the structural fix: it ensures no example that would overflow after image expansion ever reaches the processor's truncation path. At `max_length=8192` with `max_train_images=4`, the filter drops only 4 of 878K FineWeb-Edu rows (0.00%); smoltalk, densefusion, and librispeech lose 0 rows. The 2048→8192 raise is necessary because 4 images alone consume ~1128 expanded tokens, leaving <920 tokens for text at 2048 — too tight for the FineWeb-Edu text-packing distribution (p90=1842 text tokens).

**Verification:** Smoke run on A100 with `configs/3060_full.yaml --smoke` completed 10/10 steps at `batch_size=4`, `max_length=8192`, no mismatch error, `train_loss=0.4363`, peak VRAM 9.054 GiB allocated. Three new unit tests for `_filter_training_tokens` pass; all 9 config tests pass after updating `max_length == 2048` assertions to `8192`.

**Consequences:** This supersedes the 2026-07-16 decision's "2,048-token contract unchanged" clause. The VRAM gate block on the RTX 3060 (12 GB) remains — 8192-token sequences at `batch_size=4` will increase VRAM pressure beyond the 13.9 GiB already observed at 2048. The 3060 config will need either `batch_size=1`, a lower `max_length`, or larger-VRAM hardware. The A100 path (`configs/full.yaml`) is unaffected. Future dataset rebuilds should set `max_output_tokens` in FineWeb-Edu preprocessing to cap assistant targets, reducing the reliance on the pre-collator filter for extreme outliers.